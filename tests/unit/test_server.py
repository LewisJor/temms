...
""".lstrip(),
    ...
    def test_hub_ui_renders_operator_console_and_assigns_rollout(
        ...
        assert "Admin Workbench" in page.text
        assert "Guided Deploy" in page.text
        assert "Use Deploy for the normal MLflow-to-edge path" in page.text
        assert "data-theme-toggle" in page.text
        assert "theme-app" in page.text
        assert "edge-online" in page.text
        assert "pkg-vision-ui" in page.text
        assert "Build & Register" in page.text
        assert "Runtime Validation" in page.text
        assert "Benchmark Evidence" in page.text
        assert "model-vision-ui" in page.text
        assert "9.5" in page.text
        assert "Preview" in page.text

        operate_page = hub_ui_client.get("/ui/operate")
        assert operate_page.status_code == 200
        assert "Deploy A Model To Edge" in operate_page.text
        assert "Active Mission" in operate_page.text
        assert "Deployment Chain" in operate_page.text
        assert "MLflow Registry" in operate_page.text
        assert "Signed Package" in operate_page.text
        assert "Runtime Image" in operate_page.text
        assert "Edge Device" in operate_page.text
        assert "Rollout Evidence" in operate_page.text
        assert "Setup In Progress" in operate_page.text
        assert "Validate against a runtime image" in operate_page.text
        assert "Prove it runs" in operate_page.text
        assert "Guided Deployment Flow" in operate_page.text
        assert "Only the next required action opens by default" in operate_page.text
        assert "Build Signed Package" in operate_page.text
        assert "Run Validation" in operate_page.text
        assert "Assign Deployment" in operate_page.text
        assert "Preview Evidence" in operate_page.text
        assert "Runtime Images" in operate_page.text
        assert "Admin Workbench" in operate_page.text
        assert "Mission Readiness" in operate_page.text
        assert "Mission snapshot" in operate_page.text
        assert "Proof checklist" in operate_page.text
        assert "Recent evidence" not in operate_page.text
        assert "Package trust" in operate_page.text
        assert "signed package, strict metadata" in operate_page.text
        assert "Runtime validation" in operate_page.text
        assert "run package in target image" in operate_page.text
        assert "Edge action" in operate_page.text
        assert "Performance evidence" in operate_page.text
        assert "edge-online" in operate_page.text
        assert "pkg-vision-ui" in operate_page.text
        assert "pkg-vision-ui v1.0.0" in operate_page.text
        assert "temms-x86_64-cpu" in operate_page.text
        assert "needs validation" in operate_page.text
        ...
        validated_operate_page = hub_ui_client.get("/ui/operate")
        assert validated_operate_page.status_code == 200
        assert "Ready To Assign" in validated_operate_page.text
        assert "validated" in validated_operate_page.text
        assert passing_validation["validation_id"] in validated_operate_page.text
        ...
        focused_operate_page = hub_ui_client.get(
            "/ui/operate",
            params={
                "package_id": "pkg-alt-ui",
                "runtime_target_id": "temms-x86_64-cpu",
        ...
        assert focused_operate_page.status_code == 200
        assert "Focus Mission" in focused_operate_page.text
        assert "pkg-alt-ui v2.0.0" in focused_operate_page.text
        assert "needs verified signature and strict metadata" in focused_operate_page.text
        assert "run package in target image" in focused_operate_page.text
        assert passing_validation["validation_id"] not in focused_operate_page.text
        ...
        resumed_operate_page = hub_ui_client.get("/ui/operate")
        assert resumed_operate_page.status_code == 200
        assert "Saved draft:" in resumed_operate_page.text
        assert "pkg-alt-ui" in resumed_operate_page.text
        assert "edge-online on temms-x86_64-cpu" in resumed_operate_page.text
        assert "pkg-alt-ui v2.0.0" in resumed_operate_page.text
        assert "needs verified signature and strict metadata" in resumed_operate_page.text
        assert passing_validation["validation_id"] not in resumed_operate_page.text

        root_page = hub_ui_client.get("/ui/")
        assert root_page.status_code == 200
        assert "Deploy A Model To Edge" in root_page.text
        assert "Deployment Chain" in root_page.text
        assert "Guided Deployment Flow" in root_page.text
        assert "Runtime Images" in root_page.text

        runtime_catalog = hub_ui_client.get("/ui/runtimes")
        assert runtime_catalog.status_code == 200
        assert "Runtime Images" in runtime_catalog.text
        assert "Target Runtime Environments" in runtime_catalog.text
        assert "Target Environments" in runtime_catalog.text
        assert "Register Runtime Image" in runtime_catalog.text
        assert "temms/agent:inference-amd64" in runtime_catalog.text
        assert "Preview Command" in runtime_catalog.text
    ...
    def test_operate_ui_enrolls_simulated_edge_device(
        self,
        temp_dir,
        slot_manager,
        condition_store,
        ...
        """Test Operate UI can seed a simulated edge device with runtime inventory."""
        hub = HubLiteStore(temp_dir / "hub_lite_operate_enroll.json")
        hub_app = create_app(
            slot_manager=slot_manager,
            condition_store=condition_store,
            policy_engine=policy_engine,
        ...
        page = hub_ui_client.get("/ui/operate")

        assert page.status_code == 200
        assert "Enroll Sim Edge" in page.text
        assert "Enroll or start a device first" in page.text
    ...
    def test_hub_ui_airgap_export_and_import_bundle(
        ...
        assert page.status_code == 200
        assert "Air-gap Bundle" in page.text
        assert "Export Bundle" in page.text
        assert "Import Bundle" in page.text
...
class TestHubLiteEndpoints:
    ...
    def test_enrollment_catalog_rollout_status_and_airgap(self, hub_client):
        ...
        assert imported.status_code == 200
        assert imported.json()["imported"]["devices"] == 1
    ...
    def test_apply_rollout_imports_package_and_activates_slot(
        ...
        hub.assign_rollout("edge-1", "pkg-apply", slot="vision", rollout_id="rollout-apply")
        slot_manager.create_slot(name="vision", description="Vision", required=True)
        inference_runtime.load_model = AsyncMock(return_value=True)
        importer_kwargs = {}
...
""".lstrip()
        ...
        assert {
            policy.metadata.name for policy in policy_engine.list_policies()
        } == {"signed-apply-policy"}
    ...
    def test_evidence_export_includes_fleet_audit_and_benchmarks(
        ...
        assert bundle["rollout_events"][0]["actor"] == "operator:alice"
