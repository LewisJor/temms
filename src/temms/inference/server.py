...
def _mission_package_request_payload(request: MissionPackagePlanRequest) -> Dict[str, Any]:
    ...
    mission_yaml = str(payload.get("mission_yaml") or "")
    if not mission_yaml:
        return payload

    yaml_payload = _load_mission_yaml_payload(mission_yaml)
    if not yaml_payload:
        return payload
    ...
    for field_name, yaml_paths in MISSION_PACKAGE_YAML_FIELD_PATHS.items():
        ...
        if field_name in MISSION_PACKAGE_YAML_FLOAT_FIELDS:
            ...
            continue
        payload[field_name] = str(yaml_value)
...
def _coerce_mission_yaml_float(value: Any) -> Optional[float]:
    if isinstance(value, bool):
        return None
    if isinstance(value, (int, float)):
        return float(value)
